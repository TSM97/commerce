...
export default function ArticlesUploadForm() {
  ...
  const titleRefEL = useRef<HTMLInputElement>(null);
  const shortDescRefEL = useRef<HTMLInputElement>(null);
  const fullDescRefEL = useRef<HTMLTextAreaElement>(null);
  ...
  const [showGreekForm, setShowGreekForm] = useState(false);
  const [forms, setForms] = useState({ en: undefined, el: undefined });
  ...
  const handleCheck = () => {
    setShowGreekForm(!showGreekForm);
  };
  ...
  const handleOnSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const articleData = {
        ...
        ...(titleRefEL.current?.value ||
        shortDescRefEL.current?.value ||
        fullDescRefEL.current?.value
          ? {
              el: {
                title: titleRefEL.current?.value || '',
                shortDescription: shortDescRefEL.current?.value || '',
                fullDescription: fullDescRefEL.current?.value || '',
              },
            }
          : {}),
